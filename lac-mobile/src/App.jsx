...
const Header = ({ title, onBack, right }) => (
  <header className="flex items-center gap-3 px-4 py-3.5 bg-[#0B1426]/95 backdrop-blur-xl border-b border-white/[0.06] sticky top-0 z-10">
...
const StatBox = ({ icon, label, value, color = 'text-cyan-400', small }) => (
  <div className={`bg-white/[0.04] rounded-2xl ${small ? 'p-2.5' : 'p-3'} text-center border border-white/[0.06]`}>
    {icon && <div className="text-lg mb-1">{icon}</div>}
    <p className={`font-bold ${small ? 'text-base' : 'text-xl'} ${color}`}>{value}</p>
    <p className="text-[#B8E6FF]/40 text-[10px] mt-0.5">{label}</p>
  </div>
);
...
const ListItem = React.memo(({ icon, title, sub, right, onClick, badge }) => (
  <button onClick={onClick} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-white/[0.03] active:bg-white/[0.04] transition border-b border-white/[0.06]/30">
    ...
    <div className="flex-1 min-w-0 text-left">
      ...
      {sub && <p className="text-[#B8E6FF]/40 text-xs truncate">{sub}</p>}
    </div>
...
const Empty = ({ emoji, text, sub }) => (
  <div className="flex flex-col items-center justify-center py-16 px-4 opacity-60">
    <span className="text-4xl mb-3">{emoji}</span>
    <p className="text-[#B8E6FF] text-sm font-medium text-center">{text}</p>
    {sub && <p className="text-[#B8E6FF]/40 text-xs mt-1 text-center">{sub}</p>}
  </div>
);

const LevelBadge = ({ level }) => {
  const configs = [
    { from:'#6B7280', to:'#4B5563' },  // L0 gray
    { from:'#22D3EE', to:'#0891B2' },  // L1 cyan
    { from:'#34D399', to:'#059669' },  // L2 emerald
    { from:'#60A5FA', to:'#2563EB' },  // L3 blue
    { from:'#F59E0B', to:'#D97706' },  // L4 amber
    { from:'#F87171', to:'#DC2626' },  // L5 red
    { from:'#C084FC', to:'#9333EA' },  // L6 purple
    { from:'#00E5FF', to:'#FFD700' },  // L7 GOD cyan-gold
  ];
  const c = configs[Math.min(level, 7)];
  return (
    <div style={{background:`linear-gradient(135deg, ${c.from}, ${c.to})`}}
      className="w-12 h-12 rounded-2xl flex flex-col items-center justify-center shadow-lg">
      <span className="text-white font-black text-sm leading-none">L{level}</span>
  ...
  return (
    <div className="h-full overflow-y-auto pb-4">
      <div className="flex items-center gap-3 px-4 pt-4 pb-2">
        <button onClick={onMenu} className="text-[#B8E6FF]/50 hover:text-white"><Menu className="w-5 h-5" /></button>
      ...
      <div className="mx-4">
        <Card gradient="bg-gradient-to-br from-[#0a1e42] to-[#0f1f18] border-cyan-800/30" className="text-center">
          ...
          <p className="text-[#B8E6FF]/35 text-[11px] font-mono mt-1">{sAddr(p.address)}</p>
          <div className="flex justify-center gap-4 mt-3">
            <div className="text-center"><p className="text-white font-bold">{fmt(p.balance)}</p><p className="text-[#B8E6FF]/35 text-[10px]">LAC</p></div>
            <div className="text-center"><p className="text-white font-bold">{p.tx_count||0}</p><p className="text-[#B8E6FF]/35 text-[10px]">TXs</p></div>
            <div className="text-center"><p className="text-white font-bold">{p.msg_count||0}</p><p className="text-[#B8E6FF]/35 text-[10px]">{t('messages')}</p></div>
          </div>
        </Card>
      </div>
      ...
      <div className="mx-4 mt-4 space-y-0.5">
        ...
        <ListItem icon={<Award className="w-5 h-5 text-amber-400"/>} title={t('upgradeLevel')} sub={`${levelNames[p.level??0]||'L'+(p.level??0)} → ${levelNames[(p.level??0)+1]||'?'} · ${levelCosts[p.level??0]>0 ? fmt(levelCosts[p.level??0])+' LAC' : '⚡ MAX'}`}
          onClick={upgrade} right={upg?<span className="text-xs text-[#B8E6FF]/40">…</span>:undefined} />
        ...
        <div className="h-px bg-white/[0.05]/30 my-2" />
  ...
  <div className="flex gap-1 p-1 rounded-2xl bg-white/[0.04] border border-white/[0.06]">
    {tabs.map(([id, label]) => (
      <button key={id} onClick={() => onChange(id)}
        className={`flex-1 py-2 px-3 rounded-xl text-xs font-semibold transition-all ${active===id ? 'bg-cyan-400/15 text-cyan-400 border border-cyan-400/20' : 'text-[#B8E6FF]/50 hover:text-[#B8E6FF]/80'}`}>
        {label}
      </button>
    ))}
  </div>
...
const StatBox = ({ icon, label, value, color = 'text-cyan-400', small }) => (
  <div className={`bg-white/[0.04] rounded-xl ${small ? 'p-2.5' : 'p-3'} text-center border border-white/[0.05]`}>
    <div className={`${small ? 'text-xl' : 'text-2xl'} font-bold ${color}`}>{value}</div>
    <div className="text-[#B8E6FF]/40 text-[10px] mt-0.5 flex items-center justify-center gap-1">{icon}{label}</div>
  </div>
);
...
const Empty = ({ emoji, text, sub }) => (
  <div className="flex flex-col items-center py-16 text-[#B8E6FF]/35">
  ...
  return (
    <div className="h-full flex flex-col bg-[#0a0a0a]">
      <div className="flex items-center gap-3 px-4 pt-12 pb-6">
        <button onClick={onMenu} className="text-[#B8E6FF]/50"><span className="text-xl">☰</span></button>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 gap-6">
        ...
        <div className="text-center">
          ...
          <p className="text-[#B8E6FF]/40 text-sm">Receive-only · Signing coming soon</p>
        </div>

        {addr ? (
          <div className="w-full bg-[#0B1426]/60 border border-white/[0.06] rounded-2xl p-4">
            <p className="text-[#B8E6FF]/40 text-xs mb-2 text-center">Your Bitcoin Address</p>
          ...
          <div className="w-full bg-[#0B1426]/60 border border-white/[0.06] rounded-2xl p-4 text-center">
            <p className="text-[#B8E6FF]/35 text-sm">Log in to LAC to generate address</p>
          </div>
        )}

        <div className="w-full bg-[#0B1426]/40 border border-white/[0.06]/50 rounded-2xl p-4 space-y-3">
          <p className="text-[#B8E6FF]/50 text-xs font-medium uppercase tracking-wider">Coming Soon</p>
          <div className="flex items-center gap-3 text-[#B8E6FF]/35 text-sm"><span>⚡</span><span>Send & receive BTC</span></div>
          <div className="flex items-center gap-3 text-[#B8E6FF]/35 text-sm"><span>🔄</span><span>BTC ↔ LAC atomic swaps</span></div>
          <div className="flex items-center gap-3 text-[#B8E6FF]/35 text-sm"><span>🔒</span><span>HD wallet (BIP32/39/44)</span></div>
        </div>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <div className={`rounded-xl p-3 text-xs border ${
          available === null ? 'bg-white/[0.05]/40 border-white/[0.08]/30 text-[#B8E6FF]/40' :
          available ? (hasSeed ? 'bg-white/[0.04] border-cyan-800/30 text-cyan-400' : 'bg-amber-900/20 border-amber-800/30 text-amber-400') :
        ...
        {available && hasSeed && (
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setScreen('setup')}
              className="p-4 rounded-2xl bg-white/[0.04] border border-cyan-800/30 text-center active:bg-[#0B1426]/40">
              <div className="text-2xl mb-1">🔐</div>
              <div className="text-cyan-400 text-sm font-semibold">Створити</div>
              <div className="text-[#B8E6FF]/40 text-[10px]">Новий bundle</div>
            </button>
            <button onClick={() => setScreen('recover')} disabled={bundles.length === 0}
              ...
              <div className="text-[#B8E6FF]/40 text-[10px]">{bundles.length} bundle{bundles.length !== 1 ? 's' : ''}</div>
            </button>
          </div>
        )}
        ...
        {bundles.length > 0 && (
          <div>
            <div className="text-[#B8E6FF]/40 text-xs font-medium mb-2">Ваші bundles</div>
            {bundles.map(b => (
              <button key={b.bundle_id} onClick={() => { setSelBundle(b); setScreen('bundle'); }}
                className="w-full flex items-center justify-between p-3 rounded-xl bg-white/[0.04]/80 border border-white/[0.07] mb-2 active:bg-white/[0.04]">
                <div className="text-left">
                  <div className="text-white text-sm font-medium">{b.label || 'Bundle'}</div>
                  <div className="text-[#B8E6FF]/40 text-[10px]">{b.n} локацій · threshold {b.threshold} · {b.has_canary ? '🪤' : ''}{b.has_dms ? '⏰' : ''}</div>
                </div>
                <ChevronRight className="w-4 h-4 text-[#B8E6FF]/35" />
              </button>
            ))}
          </div>
        )}
        ...
        {/* How it works */}
        <div className="bg-white/[0.04]/60 border border-white/[0.07] rounded-xl p-3 text-xs text-[#B8E6FF]/40 space-y-1">
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="text-center py-6">
          ...
          <div className="text-[#B8E6FF]/40 text-sm mt-1">{result.n} локацій · threshold {result.threshold}</div>
          <div className="mt-3 bg-black/30 rounded-xl p-3">
            <div className="text-[#B8E6FF]/40 text-xs mb-1">Bundle ID</div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {/* Step 1: Secret */}
        {step === 1 && <>
          <div className="text-[#B8E6FF]/50 text-sm">Що хочеш захистити?</div>
          <div className="grid grid-cols-2 gap-2">
            {[['seed','🔑 LAC Seed'],['custom','✏️ Свій текст']].map(([v,l]) => (
              <button key={v} onClick={() => setSecretType(v)}
                className={`p-3 rounded-xl border text-sm transition-all ${secretType===v?'border-cyan-500 bg-white/[0.04] text-cyan-400':'border-white/[0.06] text-[#B8E6FF]/40'}`}>
                {l}
              </button>
            ))}
          </div>
          {secretType === 'seed' && (
            <div className={`rounded-xl p-3 text-xs border ${localStorage.getItem('lac_seed')
              ? 'bg-white/[0.03] border-cyan-800/20 text-cyan-400'
          ...
          {secretType === 'custom' && (
            <textarea value={customSecret} onChange={e => setCustomSecret(e.target.value)}
              ...
              className="w-full bg-white/[0.04] border border-white/[0.07] rounded-xl p-3 text-white text-sm resize-none h-24" />
          )}
        ...
        {/* Step 2: Locations */}
        {step === 2 && <>
          <div className="text-[#B8E6FF]/50 text-sm">Встанови GPS локації. Кожен шард прив'язаний до місця.</div>
          {locations.map((loc, i) => (
            <div key={i} className="bg-white/[0.04]/80 border border-white/[0.07] rounded-xl p-3 space-y-2">
              <div className="flex items-center justify-between">
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${canaryIdx===i?'bg-amber-900/40 text-amber-400':'bg-white/[0.05] text-cyan-500'}`}>
                  {canaryIdx===i ? '🪤 Canary' : `#${i+1}`}
                </span>
                <div className="flex gap-2">
                  <button onClick={() => setCanaryIdx(canaryIdx===i ? null : i)}
                    className={`text-[10px] px-2 py-1 rounded-lg border ${canaryIdx===i?'border-amber-700/50 text-amber-400':'border-white/[0.08]/30 text-[#B8E6FF]/35'}`}>
                    🪤 trap
                  </button>
                  {locations.length > 2 && (
                    <button onClick={() => removeLocation(i)} className="text-[#B8E6FF]/35 text-xs">✕</button>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <input value={loc.lat} onChange={e => setLoc(i,'lat',e.target.value)}
                  placeholder="Широта" className="bg-white/[0.04] border border-white/[0.07] rounded-xl px-3 py-2 text-white text-sm" />
                <input value={loc.lon} onChange={e => setLoc(i,'lon',e.target.value)}
                  placeholder="Довгота" className="bg-white/[0.04] border border-white/[0.07] rounded-xl px-3 py-2 text-white text-sm" />
              </div>
              <button onClick={() => getGPS(i)} disabled={locating===i}
                className="w-full text-xs py-2 rounded-lg bg-white/[0.04] border border-white/[0.07] text-cyan-500 disabled:opacity-50">
          ...
          {locations.length < 8 && (
            <button onClick={addLocation}
              className="w-full py-2 rounded-xl border border-dashed border-white/[0.07] text-cyan-700 text-sm">
              + Додати локацію
            </button>
          )}
          <div className="bg-white/[0.04]/60 rounded-xl p-3">
            <div className="text-[#B8E6FF]/50 text-xs mb-2">Мінімум локацій для відновлення</div>
            <div className="flex gap-2">
              {Array.from({length: locations.length-1}, (_,i) => i+2).map(v => (
                <button key={v} onClick={() => setThreshold(v)}
                  className={`px-3 py-1 rounded-lg text-sm ${threshold===v?'bg-cyan-600 text-white':'bg-white/[0.04] border border-white/[0.06] text-[#B8E6FF]/40'}`}>
                  {v}
                </button>
              ))}
            </div>
            <div className="text-[#B8E6FF]/35 text-xs mt-1">Будь-які {threshold} з {locations.length} локацій</div>
          </div>
        ...
        {/* Step 3: Confirm */}
        {step === 3 && <>
          <div className="text-[#B8E6FF]/50 text-sm">Перевір і підтвердь</div>
          <div className="bg-white/[0.04]/60 rounded-xl p-3 text-xs text-[#B8E6FF]/50 space-y-1">
          ...
          {log ? (
            <div className="bg-black/40 rounded-xl p-3 text-[10px] font-mono text-[#B8E6FF]/50 whitespace-pre-wrap break-all">{log}</div>
          ) : null}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="rounded-2xl bg-[#0d1e3a] border border-cyan-800/30 p-4 space-y-1">
          {[
            ...
            <div key={k} className="flex justify-between items-center py-2 border-b border-white/[0.06]/20">
              <span className="text-[#B8E6FF]/40 text-sm">{k}</span>
              <span className="text-white text-sm">{v}</span>
            </div>
          ))}
        </div>
        <Btn onClick={onDMS} className="w-full bg-white/[0.04] border border-cyan-800/40 text-cyan-400">
...
const NaginiRecover = ({ onBack, bundles }) => {
  ...
  if (result) return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="rounded-2xl bg-white/[0.03] border border-cyan-800/30 p-5 text-center">
          <div className="text-5xl mb-3">🔓</div>
          <div className="text-cyan-400 font-bold text-lg">Успішно!</div>
          <div className="text-[#B8E6FF]/40 text-xs mt-1">{result.collected} з {result.threshold} шардів зібрано</div>
        </div>
        <div className="bg-white/[0.04] border border-white/[0.07] rounded-xl p-4">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[#B8E6FF]/40 text-xs">Відновлений секрет (Base64)</span>
          ...
          {(() => {
            ...
            return decoded ? (
              <div>
                <div className="text-[#B8E6FF]/40 text-xs mb-1">📋 Розшифрований текст</div>
                <div className="p-3 bg-black/30 rounded-xl text-cyan-300 text-sm font-mono break-all select-all border border-white/[0.07]"
          ...
          <div className="mt-2">
            <div className="text-[#B8E6FF]/35 text-[9px] mb-1">Raw Base64 (tap to copy)</div>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {bundles.length > 1 && !sessionId && (
          <div>
            <div className="text-[#B8E6FF]/50 text-xs mb-1">Оберіть bundle</div>
            <select value={bundleId} onChange={e => setBundleId(e.target.value)}
              className="w-full bg-white/[0.04] border border-white/[0.07] rounded-xl px-3 py-2 text-white text-sm">
        ...
        {!sessionId ? (
          <div className="space-y-3">
            ...
            {bundles.length === 0 ? (
              <div className="text-[#B8E6FF]/40 text-sm text-center py-4">Немає bundles. Спочатку створи.</div>
            ) : (
          ...
          <div className="space-y-3">
            {/* Progress */}
            <div className="bg-white/[0.04]/80 border border-white/[0.07] rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="text-[#B8E6FF]/50 text-xs">Прогрес: {collected}/{sessionInfo?.threshold} шардів</div>
                <button onClick={cancelSession} className="text-[#B8E6FF]/35 text-xs">✕ Скасувати</button>
              </div>
              <div className="flex gap-1">
                {Array.from({length: sessionInfo?.n || 3}, (_, i) => (
                  <div key={i} className={`h-3 flex-1 rounded-full transition-all ${i < collected ? 'bg-cyan-500' : 'bg-white/[0.05]'}`} />
                ))}
              </div>
            ...
            {/* Last scan result */}
            {lastResult && (
              <div className={`rounded-xl p-3 text-xs font-mono border ${lastResult.matched ? 'bg-white/[0.04] border-cyan-800/30 text-cyan-400' : 'bg-white/[0.05]/40 border-white/[0.08]/30 text-[#B8E6FF]/50'}`}>
                {lastResult.err ? (
                  ...
                  <>
                    ...
                    {lastResult.matched ? <div className="text-cyan-400 mt-0.5">✅ Шард #{lastResult.shard_index} знайдено</div>
                      : <div className="text-[#B8E6FF]/40 mt-0.5">❌ Шард не знайдено тут</div>}
                  </>
                )}
              </div>
            )}
            ...
            <div className="text-[#B8E6FF]/35 text-xs text-center">
...
const NaginiDMS = ({ onBack, bundle }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="bg-white/[0.04]/60 border border-white/[0.07] rounded-xl p-3 text-xs text-[#B8E6FF]/40">
        ...
        {status?.configured ? (
          <>
            <div className="rounded-2xl bg-[#0d1e3a] border border-cyan-800/30 p-5 text-center">
              ...
              <div className="text-[#B8E6FF]/40 text-sm">until alert fires</div>
            </div>
            {[
              ...
              <div key={k} className="flex justify-between py-2 border-b border-white/[0.06]/20">
                <span className="text-[#B8E6FF]/40 text-sm">{k}</span>
                <span className="text-white text-sm">{v}</span>
              </div>
            ))}
          ...
          <div className="space-y-3">
            ...
            <div>
              <div className="text-[#B8E6FF]/50 text-xs mb-2">Alert interval</div>
              <div className="flex gap-2">
                {[12, 24, 48, 72].map(h => (
                  <button key={h} onClick={() => setForm(f=>({...f, interval_hours: h}))}
                    className={`flex-1 py-2 rounded-xl text-sm transition-all ${form.interval_hours===h?'bg-cyan-600 text-white':'bg-white/[0.04] border border-white/[0.06] text-[#B8E6FF]/40'}`}>
                    {h}h
                  </button>
                ))}
              </div>
            ...
            <div className="bg-white/[0.03] border border-cyan-800/20 rounded-xl p-3 text-xs text-cyan-500">
...
const LoginScreen = ({ onAuth }) => {
  ...
  if (mode === 'backup') return (
    <div className="h-screen bg-[#0B1426] flex flex-col p-6">
      <div className="flex-1 flex flex-col justify-center">
        <div className="text-center mb-6">
          ...
          <p className="text-[#B8E6FF]/50 mt-2 text-sm">{t('writeSeedDown')}</p>
        </div>
        <Card className="mb-4" gradient="bg-white/[0.04] border-amber-800/30">
        ...
        {saved && <div className="mt-4"><Btn onClick={() => onAuth(gen)} color="cyan" full>{t('seedSaved')} →</Btn></div>}
      </div>
    </div>
  );
  ...
  return (
    <div className="h-screen bg-[#0B1426] flex flex-col p-6">
      <button onClick={() => setMode('welcome')} className="text-[#B8E6FF]/40 mb-6 flex items-center gap-1 text-sm"><ArrowLeft className="w-4 h-4" /> {t('back')}</button>
      <div className="flex-1 flex flex-col justify-center">
        ...
        <textarea value={imp} onChange={e => setImp(e.target.value)} rows={3}
          className="w-full bg-white/[0.04] text-cyan-400 font-mono text-sm p-4 rounded-2xl border border-white/[0.07] outline-none resize-none mb-4" placeholder={t('enterSeed')+'…'} />
        <Btn onClick={login} color="cyan" full disabled={imp.length<16} loading={loading}>{t('loginBtn')}</Btn>
      </div>
    </div>
  );
};
  ...
  if (sub) {
    ...
    return (
      <div className="w-full h-[100dvh] flex items-center justify-center sm:p-4">
        <div className="w-full h-full max-w-md sm:max-h-[850px] bg-[#0B1426] sm:rounded-3xl sm:shadow-2xl sm:shadow-cyan-500/10 overflow-hidden sm:border sm:border-white/[0.06] flex flex-col relative">
      ...
      <div className="w-full h-full max-w-md sm:max-h-[850px] bg-[#0B1426] sm:rounded-3xl sm:shadow-2xl sm:shadow-cyan-500/10 overflow-hidden sm:border sm:border-white/[0.06] flex flex-col relative">
        ...
        {menuOpen && (
          <div className="absolute inset-0 z-50 flex">
            ...
            <div className="relative w-[280px] h-full bg-[#0B1426]/98 backdrop-blur-xl border-r border-white/[0.06] overflow-y-auto z-10 shadow-2xl">
              {/* Menu Header */}
              <div className="p-5 border-b border-white/[0.07]">
                <div className="flex items-center justify-between mb-4">
                  <span className="text-white font-bold text-lg">Menu</span>
                  <button onClick={() => setMenuOpen(false)} className="text-[#B8E6FF]/40"><X className="w-5 h-5" /></button>
                </div>
                <div className="flex items-center gap-3 p-3 bg-[#0f2044] rounded-xl border border-white/[0.07]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[#B8E6FF]/40 text-[11px] font-mono truncate">{sAddr(p.address)}</p>
              ...
              <div className="py-2">
                {menuItems.map((m, i) => (
                  <button key={i} onClick={m.act} className="w-full flex items-center gap-3 px-5 py-3 text-[#B8E6FF]/70 hover:bg-white/[0.05] hover:text-white transition rounded-xl mx-2 w-[calc(100%-16px)]">
                    <m.icon className="w-5 h-5 text-[#B8E6FF]/40" /><span className="text-sm font-medium">{m.label}</span>
                  </button>
                ))}
              </div>
        ...
        {/* Bottom Navigation */}
        <nav className="bg-[#0B1426]/95 backdrop-blur-xl border-t border-white/[0.06] flex shrink-0 pb-[env(safe-area-inset-bottom)]">
          {[
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {sec === 'dm' ? (
          loading ? <div className="space-y-0">{[1,2,3,4].map(i=><div key={i} className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-900/50 animate-pulse"><div className="w-11 h-11 rounded-full bg-white/[0.05]/60 shrink-0"/><div className="flex-1 space-y-2"><div className="h-3 bg-white/[0.05]/60 rounded w-1/3"/><div className="h-2.5 bg-white/[0.05]/40 rounded w-2/3"/></div></div>)}</div> :
            ...
            <ListItem key={c.peer}
              ...
              right={
                <div className="flex flex-col items-end gap-1">
                  <span className="text-[#B8E6FF]/35 text-[11px]">{ago(c.last?.timestamp)}</span>
          ...
          loading ? <div className="space-y-0">{[1,2,3].map(i=><div key={i} className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-900/50 animate-pulse"><div className="w-11 h-11 rounded-full bg-white/[0.05]/60 shrink-0"/><div className="flex-1 space-y-2"><div className="h-3 bg-white/[0.05]/60 rounded w-1/4"/><div className="h-2.5 bg-white/[0.05]/40 rounded w-1/3"/></div></div>)}</div> :
...
const MsgImage = React.memo(({ url }) => {
  ...
  if (retries >= 3) return <p className="text-[#B8E6FF]/35 text-xs italic mt-1">🖼 Image expired</p>;
  const handleError = () => {
  ...
  return (
    <div className="flex items-center gap-2 mt-1 px-3 py-2 bg-black/20 rounded-xl min-w-[180px]">
      ...
      <div className="flex-1">
        ...
        <p className="text-[9px] text-[#B8E6FF]/40 mt-0.5">{fmt(duration * progress)} / {fmt(duration)}</p>
      </div>
      <Volume2 className="w-3.5 h-3.5 text-[#B8E6FF]/40 shrink-0" />
    </div>
  );
});
  ...
  return (
    <button
      ...
      className={`w-9 h-9 flex items-center justify-center disabled:opacity-30
        ${recording ? 'text-red-400 scale-125' : 'text-[#B8E6FF]/40 hover:text-amber-400'}
...
const ChatView = ({ peer, onBack, profile }) => {
  ...
  return (
    <div className="h-full bg-[#0B1426] flex flex-col">
      <Header title={<span className="flex items-center gap-2">{peerName}{peerOnline && <span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />}</span>} onBack={onBack}
        right={<div className="flex gap-1">
          {[['regular','💬','gray'],['ephemeral','⏱','amber'],['burn','🔥','red']].map(([m,ic,c]) => 
            <button key={m} onClick={() => setMode(m)} className={`px-2 py-1 rounded-lg text-[10px] font-medium transition ${mode===m?`bg-${c}-600/20 text-${c}-400 border border-${c}-600/30`:'bg-white/[0.05]/50 text-[#B8E6FF]/35 border border-transparent'}`}>{ic}</button>
          )}
        </div>} />
          ...
          return (
            <div key={i} className={`flex flex-col ${mine?'items-end':'items-start'}`}>
              <div onClick={() => { if(!burned) setReplyTo({text:m.text||m.message,from:m.from||sAddr(m.from_address)}); }}
                ...
                className={`max-w-[78%] px-3.5 py-2 rounded-2xl cursor-pointer active:opacity-80 ${burned?'bg-[#0B1426]/50 border border-white/[0.06]':mine?'bg-gradient-to-br from-cyan-600 to-cyan-700 text-white rounded-br-sm':'bg-[#0f2044] text-gray-100 rounded-bl-sm border border-white/[0.07]'}`}>
                {!mine && <p className="text-purple-400 text-[11px] font-medium mb-0.5">{m.from||sAddr(m.from_address)}</p>}
                {m.reply_to && <div className={`text-[11px] px-2 py-1 rounded-lg mb-1.5 border-l-2 ${mine?'bg-cyan-800/30 border-cyan-400/40':'bg-white/[0.05]/50 border-purple-400/40'}`}><p className={`font-medium text-[10px] ${mine?'text-cyan-300/70':'text-purple-400/70'}`}>{m.reply_to.from}</p><p className={`truncate ${mine?'text-cyan-200/50':'text-[#B8E6FF]/50'}`}>{m.reply_to.text}</p></div>}
                {/* Media rendering */}
                {(() => {
                  ...
                  return <p className={`text-[14px] leading-snug break-words ${burned?'text-[#B8E6FF]/35 italic':''}`} style={{overflowWrap:'anywhere'}}>{txt}</p>;
                })()}
                ...
                <div className={`flex items-center gap-1.5 mt-0.5 ${mine?'justify-end':''}`}>
                  ...
                  <span className={`text-[10px] ${mine?'text-cyan-300/50':'text-[#B8E6FF]/35'}`}>{ago(m.timestamp)}</span>
              ...
              {hasRxn && <div className="flex gap-1 mt-0.5 px-1">{Object.entries(rxn).map(([em,addrs]) =>
                <button key={em} onClick={() => doReact(em)} className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-white/[0.05]/60 border border-white/[0.08]/30 text-[11px] hover:bg-white/[0.07]/60">
                  <span>{em}</span>{addrs.length > 1 && <span className="text-[#B8E6FF]/50 text-[9px]">{addrs.length}</span>}
                </button>
              )}</div>}
              {/* Emoji picker */}
              {reactTo === i && <div className="flex gap-1 mt-1 px-1 py-1 rounded-xl bg-white/[0.05]/90 border border-white/[0.08]/40 shadow-lg">
      ...
      {/* Reply bar */}
      {replyTo && <div className="flex items-center gap-2 px-3 py-2 bg-white/[0.04] border-t border-white/[0.07]">
        <div className="flex-1 border-l-2 border-cyan-500 pl-2 min-w-0"><p className="text-cyan-400 text-[10px] font-medium">{replyTo.from}</p><p className="text-[#B8E6FF]/50 text-[11px] truncate">{replyTo.text}</p></div>
        <button onClick={() => setReplyTo(null)} className="text-[#B8E6FF]/35 hover:text-[#B8E6FF]/50 shrink-0"><X className="w-4 h-4" /></button>
      </div>}
      {/* Image preview before send */}
      {imgPreview && (
        <div className="flex items-center gap-2 px-3 py-2 bg-white/[0.04] border-t border-amber-900/30">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[#B8E6FF]/40 text-[10px]">Image will self-destruct</p>
          </div>
          ...
          <button onClick={() => setImgPreview(null)} className="text-[#B8E6FF]/35 hover:text-[#B8E6FF]/50"><X className="w-4 h-4" /></button>
        </div>
      )}
      ...
      {voice.recording && (
        <div className="flex items-center gap-3 px-4 py-2 bg-red-900/20 border-t border-red-900/30">
          ...
          <button onClick={voice.cancel} className="text-[#B8E6FF]/40 text-xs">Cancel</button>
          <button onClick={handleVoiceStop} className="px-3 py-1 bg-red-600 text-white text-xs rounded-lg">⏹ Send</button>
        </div>
      )}
      <div className="p-2.5 bg-white/[0.04] border-t border-white/[0.07]">
        {/* Hidden file input */}
        ...
        <div className="flex gap-2 items-end">
          ...
          <button onClick={() => fileInputRef.current?.click()} disabled={uploading || voice.recording}
            ...
            className="w-9 h-9 flex items-center justify-center text-[#B8E6FF]/40 hover:text-cyan-400 disabled:opacity-30">
          ...
          <input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key==='Enter'&&!e.shiftKey&&send()}
            className="flex-1 bg-white/[0.04] text-white px-4 py-2.5 rounded-2xl text-sm outline-none border border-white/[0.07] focus:border-cyan-600/40 placeholder-gray-600"
    ...
    <div className="h-full bg-[#0B1426] flex flex-col">
      <Header title={group.name} onBack={onBack} right={
        <div className="flex items-center gap-2">
          <button onClick={() => { cp(gid); toast.success(t('groupCopied')); }} className="p-1.5 rounded-lg bg-white/[0.05] text-[#B8E6FF]/50 active:bg-white/[0.07]"><Copy className="w-3.5 h-3.5" /></button>
        ...
        {posts.map((p,i) => { const mine=p.from_address===profile?.address;
          ...
          <div key={stableKey} className={`flex flex-col ${mine?'items-end':'items-start'}`}>
            <div onClick={() => setReplyTo({text:p.text||p.message, from:p.from||'Anon'})}
              ...
              className={`max-w-[78%] px-3.5 py-2 rounded-2xl cursor-pointer active:opacity-80 ${mine?'bg-gradient-to-br from-cyan-600 to-cyan-700 text-white rounded-br-sm':'bg-[#0f2044] text-gray-100 rounded-bl-sm border border-white/[0.07]'}`}>
              {!mine && <p className="text-purple-400 text-[11px] font-medium mb-0.5">{p.from||'Anon'}</p>}
              {p.reply_to && <div className={`text-[11px] px-2 py-1 rounded-lg mb-1.5 border-l-2 ${mine?'bg-cyan-800/30 border-cyan-400/40':'bg-white/[0.05]/50 border-purple-400/40'}`}><p className={`font-medium text-[10px] ${mine?'text-cyan-300/70':'text-purple-400/70'}`}>{p.reply_to.from}</p><p className={`truncate ${mine?'text-cyan-200/50':'text-[#B8E6FF]/50'}`}>{p.reply_to.text}</p></div>}
              {(() => {
              ...
              <p className={`text-[10px] mt-0.5 ${mine?'text-cyan-300/50':'text-[#B8E6FF]/35'}`}>{ago(p.timestamp)}</p>
            </div>
            {hasRxn && <div className="flex gap-1 mt-0.5 px-1">{Object.entries(rxn).map(([em,addrs]) =>
              <button key={em} onClick={() => doReact(p,em)} className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-white/[0.05]/60 border border-white/[0.08]/30 text-[11px] hover:bg-white/[0.07]/60">
                <span>{em}</span>{addrs.length > 1 && <span className="text-[#B8E6FF]/50 text-[9px]">{addrs.length}</span>}
              </button>
            )}</div>}
            {reactTo === i && <div className="flex gap-1 mt-1 px-1 py-1 rounded-xl bg-white/[0.05]/90 border border-white/[0.08]/40 shadow-lg">
      ...
      {imgPreview && (
        <div className="px-3 py-2 bg-white/[0.04] border-t border-white/[0.07]">
          <div className="relative inline-block">
            <img src={imgPreview.url} alt="preview" className="h-20 rounded-xl object-cover" />
            <button onClick={() => setImgPreview(null)} className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-[#0B1426] rounded-full flex items-center justify-center"><X className="w-3 h-3 text-[#B8E6FF]/70" /></button>
          </div>
      ...
      <div className="p-2.5 bg-white/[0.04] border-t border-white/[0.07]">
        <div className="flex gap-2 items-end">
          <label className="w-9 h-9 flex items-center justify-center text-[#B8E6FF]/40 hover:text-[#B8E6FF]/70 cursor-pointer shrink-0">
          ...
          <input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => e.key==='Enter'&&!e.shiftKey&&send()}
            className="flex-1 bg-white/[0.04] text-white px-4 py-2.5 rounded-2xl text-sm outline-none border border-white/[0.07] placeholder-gray-600" placeholder={t('message')+'…'} />
...
const NewChat = ({ onBack, onGo }) => {
  ...
  const { t } = useT();
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title={t('newMessage')} onBack={onBack} />
    <div className="p-4"><p className="text-[#B8E6FF]/40 text-xs mb-2">{t('enterAddr')}</p>
      <Input value={to} onChange={setTo} placeholder="@alice or lac1…" mono />
      <div className="mt-4"><Btn onClick={() => to.trim()&&onGo({address:to.trim(),name:to.trim()})} color="cyan" full disabled={!to.trim()}>{t('startChat')}</Btn></div>
    </div></div>);
};
...
const NewGroup = ({ onBack, onDone }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="Create Group" onBack={onBack} />
    <div className="p-4 space-y-4">
      <div><label className="text-[#B8E6FF]/40 text-xs mb-1 block">Name</label><Input value={name} onChange={setName} placeholder="Group name" /></div>
      <div><label className="text-[#B8E6FF]/40 text-xs mb-2 block">Type</label>
        <div className="space-y-2">
          {types.map(([id,l,d]) => (
            <button key={id} onClick={() => setType(id)} className={`w-full p-3 rounded-xl border text-left transition ${type===id?'border-cyan-500 bg-cyan-600/10':'border-white/[0.06] bg-white/[0.04]'}`}>
              <p className="text-white text-sm font-medium">{l}</p><p className="text-[#B8E6FF]/40 text-[10px] mt-0.5">{d}</p>
            </button>))}
        </div>
      </div>
      <Btn onClick={go} color="cyan" full disabled={!name.trim()} loading={ld}>Create Group</Btn>
    </div></div>);
};
...
const ReferralView = ({ onBack }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="🤝 Referral" onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4">
      {!data ? <p className="text-[#B8E6FF]/35 text-center py-8">{t('loading')}</p> : <>
        {/* Your code */}
        <Card gradient="bg-gradient-to-br from-purple-900/20 to-[#0f1f18] border-purple-800/20" className="mb-3">
          ...
          <div className="bg-[#0B1426] p-3 rounded-xl border border-purple-900/20 text-center">
            <p onClick={() => cp(data.code)} className="text-purple-300 font-mono text-xl font-bold cursor-pointer select-all">{data.code}</p>
            <p className="text-[#B8E6FF]/35 text-[10px] mt-1">{t('tapCopy')}</p>
          </div>
          ...
          <div className="mt-2 bg-[#0B1426] p-2 rounded-lg border border-white/[0.06]/30">
            <p className="text-[9px] text-[#B8E6FF]/35 mb-1">Referral link:</p>
          ...
          <div className="grid grid-cols-3 gap-2 mt-3">
            <div className="text-center"><p className="text-purple-400 font-bold text-lg">{data.referrals||0}</p><p className="text-[#B8E6FF]/35 text-[9px]">Invited</p></div>
            <div className="text-center"><p className="text-amber-400 font-bold text-lg">{(data.referrals||0)*25}</p><p className="text-[#B8E6FF]/35 text-[9px]">LAC earned</p></div>
            <div className="text-center"><p className="text-red-400 font-bold text-lg">{fmt(data.boost_burned||0)}</p><p className="text-[#B8E6FF]/35 text-[9px]">Boosted</p></div>
          </div>
        </Card>
        ...
        {!data.invited_by && <Card className="mb-3">
          ...
          <p className="text-[#B8E6FF]/35 text-[10px] mb-2">Got a code? Enter it for +50 LAC bonus!</p>
          <div className="flex gap-2">
            <input value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="REF-XXXXXXXX"
              className="flex-1 bg-white/[0.04] text-purple-400 font-mono text-sm px-3 py-2 rounded-xl border border-purple-900/30 outline-none" />
        ...
        <Card className="mb-3">
          ...
          <p className="text-[#B8E6FF]/35 text-[10px] mb-2">Burn LAC to boost your tier. 10,000+ = VIP</p>
          <div className="flex gap-2">
            <input value={boost} onChange={e => setBoost(e.target.value)} placeholder="Amount (min 100)" type="number"
              className="flex-1 bg-white/[0.04] text-red-400 font-mono text-sm px-3 py-2 rounded-xl border border-red-900/30 outline-none" />
        ...
        <Card className="mb-3">
          ...
          {[['genesis','🏆','First 100 wallets','amber'],['early','⚡','First 1,000 wallets','purple'],['growth','🌱','Standard referrers','emerald'],['vip','💎','10+ referrals or 10K burned','blue']].map(([id,ic,desc,c]) =>
            <div key={id} className={`flex items-center gap-3 py-2 border-b border-white/[0.06]/20 ${data.tier===id?'opacity-100':'opacity-50'}`}>
              <span className="text-lg">{ic}</span>
              <div className="flex-1"><p className={`text-${c}-400 text-xs font-semibold capitalize`}>{id}</p><p className="text-[#B8E6FF]/35 text-[10px]">{desc}</p></div>
        ...
        {board && board.leaderboard?.length > 0 && <Card>
          ...
          <p className="text-[#B8E6FF]/35 text-[10px] mb-2">{board.total_referrers} referrers · {board.total_referrals} total invites</p>
          {board.leaderboard.slice(0,10).map((b,i) => {
            const bt = tierInfo[b.tier||'growth'];
            return <div key={i} className="flex items-center justify-between py-1.5 border-b border-white/[0.06]/20">
              <div className="flex items-center gap-2"><span className="text-[#B8E6FF]/35 text-xs w-5">#{i+1}</span><span className="text-[#B8E6FF]/40 font-mono text-[10px]">{b.code}</span></div>
  ...
  return (
    <div className="h-full overflow-y-auto pb-4">
      ...
      <div className="mx-4 mt-3">
        ...
        >
          <span className="text-[#B8E6FF]/40 text-xs font-medium">{t('recentTx')}</span>
          <div className="flex items-center gap-2">
            <button onClick={e => { e.stopPropagation(); onNav({type:'txs'}); }} className="text-cyan-500 text-[11px]">{t('viewAll')}</button>
            <ChevronDown className={`w-4 h-4 text-[#B8E6FF]/35 transition-transform ${txOpen ? 'rotate-180' : ''}`} />
          </div>
        </button>
...
const MiningMini = () => {
  ...
  if (!d) return <p className="text-[#B8E6FF]/35 text-xs">Loading…</p>;
  const earned = d.total_mined || 0;
  const mined = d.count || 0;
  return (
    <div className="flex items-center justify-between">
      <div>
        <p className="text-2xl font-bold text-cyan-400">{earned > 0 ? fmt(earned) : '0'} <span className="text-sm text-[#B8E6FF]/40">LAC earned</span></p>
      </div>
      <div className={`px-2.5 py-1 rounded-lg text-xs font-semibold ${mined>0?'bg-[#0B1426]/40 text-cyan-400':'bg-amber-900/30 text-amber-400'}`}>
...
const LevelBonuses = ({ level }) => {
  ...
  const isGod = level >= 7;
  return (
    <div className={`mt-2 px-3 py-1.5 rounded-lg text-xs ${isGod ? 'bg-amber-500/10 border border-amber-500/30 text-amber-400' : 'bg-white/[0.04] border border-white/[0.07] text-cyan-400/80'}`}>
      {isGod ? '⚡ ' : '✓ '}{bonus}
    </div>
  );
};
  ...
  return (
    <div>
      <div className="flex justify-between text-[11px] mb-1">
        <span className={isGod?'text-amber-400 font-bold':'text-[#B8E6FF]/40'}>{isGod?'⚡ GOD':name} (L{level})</span>
        <span className="text-[#B8E6FF]/40">{cost>0?`${fmt(balance)}/${fmt(cost)} LAC`:(isGod?'⚡ MAX LEVEL':'MAX')}</span>
        {cost>0 && <span className="text-[#B8E6FF]/40">{nextName} (L{level+1})</span>}
      </div>
      <div className="h-2 bg-white/[0.05] rounded-full overflow-hidden"><div className={`h-full ${isGod?'bg-gradient-to-r from-amber-500 to-yellow-400':'bg-gradient-to-r from-cyan-500 to-cyan-400'} rounded-full transition-all`} style={{width:`${pct}%`}} /></div>
    </div>
  );
};
...
const TxRow = ({ tx }) => {
  ...
  const icons = {received:<ArrowDownLeft className="w-4 h-4 text-[#00E5FF]"/>,sent:<ArrowUpRight className="w-4 h-4 text-[#B8E6FF]/50"/>,mined:<Zap className="w-4 h-4 text-blue-400"/>,burned:<Flame className="w-4 h-4 text-red-400"/>};
  const bgs = {received:'bg-[#00E5FF]/10',sent:'bg-white/[0.05]',mined:'bg-blue-400/10',burned:'bg-red-400/10'};
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-white/[0.04]">
      <div className={`w-9 h-9 rounded-2xl flex items-center justify-center ${bgs[tx.dir]||'bg-white/[0.05]'}`}>{icons[tx.dir]}</div>
      <div className="flex-1 min-w-0">
        <p className="text-white text-sm">{txLabel(tx)}</p>
        <p className="text-[#B8E6FF]/35 text-[11px]">{ago(tx.timestamp)}{tx.block?` · #${tx.block}`:''}</p>
      </div>
      <span className={`text-sm font-semibold ${isIn?'text-cyan-400':'text-[#B8E6FF]/50'}`}>{isIn?'+':'−'}{fmt(tx.amount)}</span>
    </div>
  );
};
...
const SendView = ({ onBack, profile, onDone }) => {
  ...
  if (!type) return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="Send LAC" onBack={onBack} />
    <div className="p-4 space-y-3">
      <button onClick={() => setType('normal')} className="w-full bg-[#0d1b35] border border-white/[0.07] rounded-2xl p-4 flex items-center gap-4 active:bg-white/[0.04]">
        <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center"><Send className="w-6 h-6 text-white" /></div>
        <div className="text-left"><p className="text-white font-semibold">Normal Transfer</p><p className="text-[#B8E6FF]/40 text-xs">Public on blockchain · 0.1 LAC fee</p></div>
      </button>
      <button onClick={() => setType('veil')} className="w-full bg-[#0d1b35] border border-white/[0.07] rounded-2xl p-4 flex items-center gap-4 active:bg-purple-900/20">
        ...
        <div className="text-left"><p className="text-white font-semibold">VEIL Transfer</p><p className="text-[#B8E6FF]/40 text-xs">Full anonymity · Ring + Phantom TXs · 1.0 LAC</p>
          <span className="flex gap-1 mt-1 flex-wrap">{['Ring Sig','Phantom','OTA','Zero-History'].map(f => <Badge key={f} color="purple">{f}</Badge>)}</span>
        </div>
      </button>
    </div></div>);

  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title={type==='veil'?'VEIL Transfer':'Normal Transfer'} onBack={() => setType(null)} />
    <div className="p-4 space-y-4">
      <div className={`rounded-xl p-3 border flex items-center gap-2 ${type==='veil'?'bg-purple-900/15 border-purple-700/30':'bg-blue-900/15 border-blue-700/30'}`}>
        ...
        <span className="text-[#B8E6FF]/50 text-xs">Fee: {type==='veil'?'1.0':'0.1'} LAC</span>
      </div>
      <div><label className="text-[#B8E6FF]/40 text-xs mb-1 block">To</label><Input value={to} onChange={setTo} placeholder="@username or lac1…" mono /></div>
      <div><label className="text-[#B8E6FF]/40 text-xs mb-1 block">Amount</label><Input value={amt} onChange={setAmt} type="number" placeholder="0" />
...
const STASHView = ({ onBack, onDone }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title={'🎒 '+t('stashTitle')} onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4">
      <Card gradient="bg-gradient-to-br from-amber-900/20 to-[#0f1f18] border-amber-800/20" className="mb-4">
        ...
        <p className="text-[#B8E6FF]/40 text-[11px] mt-1">{t('stashDesc')}</p>
        {info && <div className="mt-2 grid grid-cols-3 gap-2">
          <div><p className="text-amber-300 text-xs font-bold">{fmt(info.total_balance||0)}</p><p className="text-[#B8E6FF]/35 text-[9px]">{t('poolLac')}</p></div>
          <div><p className="text-cyan-400 text-xs font-bold">{info.active_keys||0}</p><p className="text-[#B8E6FF]/35 text-[9px]">{t('activeKeys')}</p></div>
          <div><p className="text-[#B8E6FF]/50 text-xs font-bold">{info.spent_count||0}</p><p className="text-[#B8E6FF]/35 text-[9px]">{t('redeemed')}</p></div>
        </div>}
      </Card>

      {/* Result feedback */}
      {res?.t==='dep' && res.key && (
        <Card gradient="bg-[#0B1426]/15 border-cyan-700/30" className="mb-3">
          <p className="text-cyan-400 text-sm font-bold mb-2">✅ {t('depositSuccess')}</p>
          <div className="bg-[#0B1426] p-3 rounded-lg border border-white/[0.07]">
            <p className="text-[9px] text-[#B8E6FF]/35 mb-1 uppercase tracking-wider">{t('stashKey')}</p>
            <p onClick={() => cp(res.key)} className="text-cyan-300 font-mono text-[12px] break-all select-all cursor-pointer leading-5">{res.key}</p>
          </div>
          <div className="flex gap-2 mt-2">
            <button onClick={() => cp(res.key)} className="flex-1 text-cyan-400 text-xs flex items-center justify-center gap-1 bg-white/[0.04] px-3 py-2 rounded-lg active:bg-[#0B1426]/40"><Copy className="w-3 h-3"/> {t('copy')}</button>
      ...
      <div className="mt-3">
        {tab==='dep' ? (
          <div className="space-y-2">
            {noms.map(n => (
              <button key={n.c} onClick={() => setNom(n.c)} className={`w-full p-3 rounded-xl border text-left ${nom===n.c?'border-amber-500 bg-amber-600/10':'border-white/[0.06] bg-white/[0.04]'}`}>
                <span className="text-white font-semibold text-sm">{fmt(n.a)} LAC</span>
                <span className="text-[#B8E6FF]/35 text-[11px] ml-2">({t('fee')}: 2 LAC)</span>
              </button>))}
          ...
          <div className="space-y-3">
            <textarea value={key} onChange={e => setKey(e.target.value)} rows={3} placeholder={t('withdrawKey')+'…'}
              className="w-full bg-white/[0.04] text-cyan-400 font-mono text-[11px] p-3 rounded-xl border border-white/[0.07] outline-none resize-none" />
            <Btn onClick={wdr} color="cyan" full loading={ld} disabled={!key.trim()}>{t('withdraw')}</Btn>
          </div>
        ) : (
          <div className="space-y-2">
            {savedKeys.length === 0 ? <Empty emoji="🔑" text={t('noKeys')} sub={t('deposit')+' STASH'} /> :
            savedKeys.map((s, i) => (
              <Card key={i} gradient={s.used ? 'bg-white/[0.05]/20 border-white/[0.08]/20' : 'bg-amber-900/10 border-amber-800/20'}>
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <p className={`font-mono text-[10px] break-all ${s.used ? 'text-[#B8E6FF]/35 line-through' : 'text-amber-300'}`}>{s.key}</p>
                    <p className="text-[#B8E6FF]/35 text-[10px] mt-1">{fmt(s.amount)} LAC · {s.used ? '✅ '+t('markUsed') : '⏳ '+t('active')}</p>
                  </div>
                  <div className="flex gap-1 ml-2 shrink-0">
                    {!s.used && <button onClick={() => { setKey(s.key); setTab('wdr'); }} className="p-1.5 bg-white/[0.04] rounded-lg" title="Use"><Download className="w-3 h-3 text-cyan-400" /></button>}
...
const TimelockView = ({ onBack, profile, onDone }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="⏰ Time-Lock" onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <Card><p className="text-[#B8E6FF]/50 text-xs">Send LAC in the future. Funds locked until X blocks pass (~10s/block).</p></Card>
      <div><label className="text-[#B8E6FF]/40 text-xs mb-1 block">Recipient</label><Input value={to} onChange={setTo} placeholder="@user or lac1…" mono /></div>
      <div><label className="text-[#B8E6FF]/40 text-xs mb-1 block">Amount (LAC)</label><Input value={amt} onChange={setAmt} type="number" placeholder="0" /></div>
      <div>
        <label className="text-[#B8E6FF]/40 text-xs mb-1 block">Send in (blocks) {delay && <span className="text-cyan-400">{estTime(parseInt(delay))}</span>}</label>
        <Input value={delay} onChange={setDelay} type="number" placeholder="e.g. 60 = ~10min" />
        <div className="flex gap-2 mt-2">
          {[['10min',60],['1hr',360],['6hr',2160],['24hr',8640]].map(([label,b]) =>
            <button key={b} onClick={() => setDelay(String(b))} className="px-3 py-1.5 bg-white/[0.04] text-cyan-400 text-[10px] rounded-lg border border-cyan-800/20 active:bg-cyan-800/30">{label}</button>
          )}
        </div>
      </div>
      <Btn onClick={go} color="cyan" full loading={ld} disabled={!to||!amt||!delay}>Create TimeLock</Btn>
      {pending.length > 0 && (<div>
        <p className="text-[#B8E6FF]/40 text-xs font-medium mt-4 mb-2">Pending Locks</p>
        {pending.map((l,i) => <Card key={i} className="mb-2"><p className="text-white text-sm">{fmt(l.amount)} LAC → {sAddr(l.to)}</p><p className="text-[#B8E6FF]/35 text-[11px]">Block #{l.unlock_block} · {l.blocks_remaining||'?'} blocks left</p></Card>)}
      </div>)}
    </div></div>);
};
...
const DeadManSwitchView = ({ onBack, profile, onDone }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="💀 Dead Man's Switch" onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <Card gradient="bg-gradient-to-br from-red-900/20 to-[#0f1f18] border-red-800/20">
        ...
        <p className="text-[#B8E6FF]/40 text-[11px] mt-1">If you don't log in for X days, automatic actions trigger: transfer funds, send messages, or wipe your wallet.</p>
      </Card>
        ...
        <Card>
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-[#B8E6FF]/40 text-[11px] mt-1">Timeout: {status.timeout_days} days</p>
              <p className="text-[#B8E6FF]/40 text-[11px]">Days remaining: <span className="text-amber-400">{status.days_remaining}</span></p>
              <p className="text-[#B8E6FF]/35 text-[10px] mt-1">{status.actions?.length||0} actions configured</p>
            </div>
            <Btn onClick={cancel} color="gray" small>Disable</Btn>
          </div>
          {status.actions?.map((a,i) => <p key={i} className="text-[#B8E6FF]/40 text-[10px] mt-1">→ {actionLabels[a.type]||a.type} {a.to ? `to ${a.to}` : ''}</p>)}
        </Card>
      ) : status?.triggered_at ? (
        <Card><p className="text-red-400 text-sm">💀 DMS was triggered</p><p className="text-[#B8E6FF]/35 text-[10px]">Actions were executed. Set up a new one if needed.</p></Card>
      ) : (
        <Card><p className="text-[#B8E6FF]/40 text-sm">No DMS configured</p></Card>
      )}
      ...
      {showSetup && (<>
        <div>
          <label className="text-[#B8E6FF]/40 text-xs mb-1 block">Trigger after (days without login)</label>
          <Input value={days} onChange={setDays} type="number" placeholder="30" />
          <div className="flex gap-2 mt-2">
            {[7,14,30,90,180].map(d => <button key={d} onClick={() => setDays(String(d))} className={`px-3 py-1.5 text-[10px] rounded-lg border ${days===String(d)?'bg-red-900/20 text-red-400 border-red-800/30':'bg-white/[0.05]/50 text-[#B8E6FF]/35 border-white/[0.08]/30'}`}>{d}d</button>)}
          </div>
        </div>

        <p className="text-[#B8E6FF]/40 text-xs font-medium">Actions (executed in order):</p>
        {actions.map((a, i) => (
          <Card key={i} className="relative">
            <button onClick={() => removeAction(i)} className="absolute top-2 right-2 text-[#B8E6FF]/35 text-xs">✕</button>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(actionLabels).map(([type, label]) =>
            <button key={type} onClick={() => addAction(type)} className="px-2.5 py-1.5 bg-white/[0.05] text-[#B8E6FF]/50 text-[10px] rounded-lg border border-white/[0.08]/30 active:bg-white/[0.07]">+ {label}</button>
          )}
        </div>
...
const MiningView = ({ onBack, profile }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="⛏️ Mining" onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4">
      <Card gradient="bg-gradient-to-br from-cyan-800/30 to-[#0f1f18] border-cyan-700/30" className="mb-4 text-center">
        ...
        <p className="text-[#B8E6FF]/40 text-xs">PoET Consensus · Weighted Lottery</p>
      </Card>
      ...
      <Card><p className="text-white text-sm font-semibold mb-2">Mining Info</p>
        {[['Block Reward','190 LAC'],['Winners/Block','19'],['Min Balance','50 LAC'],['Your Balance',fmt(p.balance)+' LAC']].map(([k,v]) => (
          <div key={k} className="flex justify-between py-1.5 border-b border-white/[0.06]/20"><span className="text-[#B8E6FF]/40 text-xs">{k}</span><span className="text-white text-xs font-medium">{v}</span></div>
        ))}
      </Card>
      {rewards.length > 0 && (<div className="mt-4">
        <p className="text-[#B8E6FF]/40 text-xs font-medium mb-2">Recent Rewards</p>
        {rewards.slice(0,10).map((r,i) => <Card key={i} className="mb-1.5"><div className="flex justify-between"><span className="text-cyan-400 text-sm font-semibold">+{fmt(r.reward||r.amount)} LAC</span><span className="text-[#B8E6FF]/35 text-[11px]">Block #{r.block}</span></div></Card>)}
      </div>)}
    </div></div>);
};
...
const TxsView = ({ onBack }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="📜 Transactions" onBack={onBack} right={<span className="text-[#B8E6FF]/35 text-xs">{show.length}</span>} />
    <div className="flex gap-1 px-4 py-2">
      {[['all','All'],['received','In'],['sent','Out'],['mined','Mined'],['burned','Burn']].map(([id,l]) => (
        <button key={id} onClick={() => setF(id)} className={`px-3 py-1 rounded-lg text-[11px] font-semibold ${f===id?'bg-cyan-600 text-white':'bg-white/[0.05] text-[#B8E6FF]/40'}`}>{l}</button>
      ))}
    </div>
    <div className="flex-1 overflow-y-auto px-4">
      {ld?<p className="text-[#B8E6FF]/35 py-8 text-center text-sm">Loading…</p>:show.length===0?<Empty emoji="📜" text="No transactions"/>:show.map((tx,i) => <TxRow key={i} tx={tx} />)}
    </div></div>);
};
...
const DiceView = ({ onBack, profile, onRefresh }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="🎲 Dice" onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4">
      {/* Balance */}
      <div className="text-center mb-4">
        <p className="text-[#B8E6FF]/40 text-xs">Your Balance</p>
        <p className="text-2xl font-bold text-white">{fmt(p.balance)} <span className="text-sm text-[#B8E6FF]/40">LAC</span></p>
      </div>
      ...
      {/* Bet amount */}
      <div className="mt-4">
        <p className="text-[#B8E6FF]/40 text-xs mb-2">Bet Amount</p>
        <div className="flex gap-2">
          <input type="number" value={bet} onChange={e => setBet(e.target.value)}
            className="flex-1 bg-white/[0.04] text-white text-center font-bold text-lg px-4 py-3 rounded-xl border border-white/[0.07] outline-none" />
          <div className="flex flex-col gap-1">
            {[10,50,100].map(v => (
              <button key={v} onClick={() => setBet(String(v))} className="px-3 py-1 bg-white/[0.05] text-[#B8E6FF]/50 text-[11px] rounded-lg active:bg-white/[0.07]">{v}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* Spinning animation */}
      {playing && <div className="text-center py-6"><p className="text-3xl animate-bounce">🎲</p><p className="text-[#B8E6FF]/40 text-xs mt-2">Rolling…</p></div>}
      ...
      {result && !playing && (
        <Card gradient={result.won?'bg-gradient-to-br from-cyan-900/30 to-[#0f1f18] border-cyan-500/30':'bg-gradient-to-br from-red-900/20 to-[#0f1f18] border-red-700/30'} className="mt-4 text-center">
          ...
          <p className="text-[#B8E6FF]/40 text-sm mt-1">
            Roll: <span className="text-white font-mono">{result.roll}</span> → <span className={`font-bold ${result.result==='RED'?'text-red-400':result.result==='BLACK'?'text-[#B8E6FF]/70':'text-cyan-400'}`}>{result.result}</span>
          </p>
      ...
      {history.length > 0 && (
        <div className="mt-4">
          <p className="text-[#B8E6FF]/40 text-xs font-medium mb-2">Recent Games</p>
          {history.slice(0,8).map((g,i) => (
            <div key={i} className="flex items-center justify-between py-1.5 border-b border-white/[0.06]/20">
              <div className="flex items-center gap-2">
                <span className="text-sm">{g.won ? '🟢' : '🔴'}</span>
                <span className="text-[#B8E6FF]/50 text-xs">{g.choice?.toUpperCase()} → {g.result}</span>
              </div>
      ...
      <Card className="mt-4" gradient="bg-white/[0.05]/20 border-white/[0.08]/30">
        <p className="text-[#B8E6FF]/35 text-[10px] leading-relaxed">
...
const ValidatorView = ({ onBack, profile, onRefresh }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="🛡️ Validator" onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4">
      {/* Status Card */}
      <Card gradient={active ? 'bg-gradient-to-br from-purple-900/40 to-[#0f1f18] border-purple-700/30' : 'bg-gradient-to-br from-[#0a1e42] to-[#0f1f18] border-cyan-800/30'} className="mb-4 text-center">
        ...
        <p className={`font-bold text-lg ${active?'text-purple-400':'text-[#B8E6FF]/50'}`}>
          {active ? 'Validator Active' : eligible ? 'Validator Ready' : 'Not Eligible'}
        </p>
        <p className="text-[#B8E6FF]/35 text-xs mt-1">
      ...
      <Card><p className="text-white text-sm font-semibold mb-2">Validator Info</p>
        {[
          ...
          <div key={k} className="flex justify-between py-1.5 border-b border-white/[0.06]/20">
            <span className="text-[#B8E6FF]/40 text-xs">{k}</span>
      ...
      {validators.length > 0 && (
        <div className="mt-4">
          <p className="text-[#B8E6FF]/40 text-xs font-medium mb-2">Active Validators ({validators.length})</p>
          {validators.map((v,i) => (
            <Card key={i} className="mb-1.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Badge color="purple">L{v.level}</Badge>
                  <span className="text-[#B8E6FF]/50 text-xs font-mono">{v.address}</span>
                </div>
                <span className="text-[#B8E6FF]/35 text-[11px]">w:{v.weight}</span>
              </div>
            </Card>
          ))}
        </div>
      ...
      {!eligible && (
        <Card className="mt-4" gradient="bg-amber-900/10 border-amber-700/20">
          ...
          <div className="space-y-2 text-xs text-[#B8E6FF]/50">
...
const UsernameView = ({ onBack, onDone }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title={'👤 '+t('registerUsername')} onBack={onBack} />
    <div className="p-4 space-y-4">
      <Card><p className="text-[#B8E6FF]/50 text-xs">{t('getYourName')}. Shorter = more expensive.</p></Card>
      <div className="flex gap-2">
        <div className="flex-1 flex items-center bg-white/[0.04] border border-white/[0.07] rounded-xl overflow-hidden">
          <span className="text-[#B8E6FF]/35 pl-3">@</span>
      ...
      <div className="flex justify-between text-[11px]"><span className="text-[#B8E6FF]/35">3-20 chars · a-z 0-9 _</span><span className="text-amber-400">{cost>0?cost+' LAC':'—'}</span></div>
      {avail===true && <Btn onClick={reg} color="cyan" full loading={ld}>Register @{name} ({cost} LAC)</Btn>}
      ...
      <Card className="mt-4"><p className="text-white text-sm font-semibold mb-2">💰 Pricing</p>
        {[['3 chars','10,000 LAC'],['4 chars','1,000 LAC'],['5 chars','100 LAC'],['6+ chars','10 LAC']].map(([k,v]) => (
          <div key={k} className="flex justify-between py-1.5 border-b border-white/[0.06]/20"><span className="text-[#B8E6FF]/40 text-xs">{k}</span><span className="text-amber-400 text-xs font-medium">{v}</span></div>
        ))}
      </Card>
    </div></div>);
};
...
const ContactsView = ({ onBack, onChat }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title={'👥 '+t('contacts')} onBack={onBack} />
    <div className="p-4">
      <div className="flex gap-2 mb-4"><Input value={addr} onChange={setAddr} placeholder={t('enterContact')} mono /><Btn onClick={add} color="cyan" small><UserPlus className="w-4 h-4"/></Btn></div>
...
const PolView = ({ onBack, profile }) => {
  ...
  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="📍 Proof-of-Location" onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      {/* Info */}
      <Card gradient="bg-gradient-to-br from-blue-900/20 to-[#0f1f18] border-blue-800/15">
        ...
        <p className="text-[#B8E6FF]/50 text-xs leading-relaxed">Prove you're in a region WITHOUT revealing exact coordinates. Your GPS data stays on your device — only the zone name goes on-chain.</p>
      </Card>
      ...
      {!coords ? (
        <Card>
          ...
          <button onClick={getLocation} disabled={status === 'detecting'}
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-white/[0.07] text-white font-bold py-3 rounded-xl text-sm">
          ...
          {error && (
            <div className="mt-2">
              ...
              <div className="mt-2 bg-amber-900/20 border border-amber-800/30 rounded-xl p-3">
                ...
                <p className="text-[#B8E6FF]/40 text-[10px] mb-2">Your device blocks GPS on HTTP. Use manual zone selection instead — you choose your zone without GPS.</p>
          ...
          {!error && <p className="text-[#B8E6FF]/35 text-[10px] mt-2 text-center">Coordinates never leave your device</p>}
          {!error && (
            <button onClick={enterManualMode} className="w-full mt-2 text-[#B8E6FF]/35 text-[10px] hover:text-[#B8E6FF]/50 py-1">
        ...
        <Card gradient="bg-gradient-to-br from-cyan-900/20 to-[#0f1f18] border-cyan-800/15">
          ...
          {manualMode ? (
            ...
            <p className="text-[#B8E6FF]/35 text-[10px] mb-3">GPS: {coords?.lat?.toFixed(4)}, {coords?.lon?.toFixed(4)} (±{Math.round(coords?.accuracy||0)}m) — stays on device</p>
          )}
          {zones.length > 0 ? (
            <div className="space-y-1.5 max-h-64 overflow-y-auto">
              {zones.map(z => (
                <button key={z} onClick={() => setSelectedZone(z)}
                  className={`w-full text-left px-3 py-2.5 rounded-xl text-sm border transition-all ${selectedZone === z ? 'bg-white/[0.05] border-cyan-500/50 text-cyan-400' : 'bg-[#0a1a14] border-white/[0.05] text-[#B8E6FF]/50'}`}>
                  {z === selectedZone ? '✅ ' : '📍 '}{z}
                </button>
              ))}
            </div>
          ) : <p className="text-[#B8E6FF]/35 text-xs">No zones detected for your location</p>}
        </Card>
        ...
        <Card>
          ...
          {msgMode && <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Message text bound to location proof..."
            className="w-full mt-2 bg-[#0a1a14] border border-white/[0.07] rounded-xl p-3 text-white text-sm h-24 resize-none" />}
        </Card>

        {/* Step 3: Generate */}
        <button onClick={createProof} disabled={!selectedZone || status === 'proving'}
          className="w-full bg-cyan-600 hover:bg-cyan-700 disabled:bg-white/[0.07] text-white font-bold py-3.5 rounded-xl text-sm">
        ...
        <Card gradient="bg-gradient-to-br from-cyan-900/20 to-[#0f1f18] border-cyan-500/30">
          ...
          <div className="space-y-2">
            <div className="flex justify-between"><span className="text-[#B8E6FF]/35 text-xs">Zone</span><span className="text-white text-sm font-semibold">{proof?.zone}</span></div>
            <div className="flex justify-between"><span className="text-[#B8E6FF]/35 text-xs">Area</span><span className="text-[#B8E6FF]/50 text-xs">~{(proof?.area_km2||0).toLocaleString()} km²</span></div>
            <div className="flex justify-between"><span className="text-[#B8E6FF]/35 text-xs">Protocol</span><span className="text-blue-400 text-xs">{proof?.protocol}</span></div>
            <div className="flex justify-between"><span className="text-[#B8E6FF]/35 text-xs">Privacy</span><span className="text-cyan-400 text-xs">Zone only — no coordinates</span></div>
            {proof?.all_zones?.length > 1 && <div><span className="text-[#B8E6FF]/35 text-xs">Also in:</span><span className="text-[#B8E6FF]/40 text-xs ml-1">{proof.all_zones.filter(z=>z!==proof.zone).join(', ')}</span></div>}
            <div className="mt-3 bg-[#0B1426] rounded-xl p-2">
              <p className="text-[#B8E6FF]/35 text-[9px] font-mono break-all">Commitment: {proof?.commitment?.slice(0,32)}...</p>
              <p className="text-[#B8E6FF]/35 text-[9px] font-mono break-all">Proof: {proof?.proof_hash?.slice(0,32)}...</p>
            </div>
          </div>
          {proof?.message_binding && <div className="mt-2 bg-amber-900/10 border border-amber-800/20 rounded-xl p-2">
            ...
            <p className="text-[#B8E6FF]/35 text-[9px] font-mono">Binding: {proof.message_binding.slice(0,32)}...</p>
          </div>}
          <button onClick={() => { setStatus('idle'); setProof(null); setCoords(null); setZones([]); }}
            className="w-full mt-4 bg-[#0a1a14] border border-white/[0.07] text-[#B8E6FF]/50 py-2.5 rounded-xl text-xs">New Proof</button>
        </Card>
      )}
...
const DashboardView = ({ onBack }) => {
  ...
  const levelNames = ["Newbie","Starter","Active","Trusted","Expert","Validator","Priority","⚡ GOD"];
  const levelColors = ["text-[#B8E6FF]/50","text-cyan-400","text-teal-400","text-cyan-400","text-amber-400","text-red-400","text-purple-400","text-yellow-300"];

  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title={"📊 "+t("dashboard")} onBack={onBack} />
    <div className="flex-1 overflow-y-auto p-4">
      {!s ? <p className="text-[#B8E6FF]/35 text-center py-8">{t("loading")}</p> : <>
        <Card gradient="bg-gradient-to-br from-cyan-900/20 to-[#0f1f18] border-cyan-800/15" className="mb-3">
          ...
          <div className="bg-[#0B1426] rounded-xl p-3 mb-2 border border-white/[0.07]">
            <p className="text-[9px] text-[#B8E6FF]/35 uppercase mb-0.5">💼 {t("onWallets")}</p>
            <p className="text-cyan-400 text-2xl font-bold">{fmt(s.on_wallets||s.circulating_supply||0)} <span className="text-sm text-[#B8E6FF]/35">LAC</span></p>
          </div>
          <div className="flex justify-between items-center py-2 border-b border-white/[0.06]/30">
        ...
        {s.top_balances && (
          <Card className="mb-3">
            ...
            {s.top_balances.slice(0,5).map((b,i) => (
              <div key={i} className="flex justify-between py-1.5 border-b border-white/[0.06]/20">
                <span className="text-[#B8E6FF]/40 text-xs">#{i+1}</span>
        ...
        <Card>
          ...
          {Object.entries(buildLevels(s.level_distribution)).map(([k, v], i) => (
            <div key={k} className="flex justify-between items-center py-1.5 border-b border-white/[0.06]/20 last:border-0">
              <div className="flex items-center gap-2">
                <span className={"text-xs font-bold "+levelColors[i]}>{k}</span>
                <span className="text-[#B8E6FF]/35 text-[10px]">{levelNames[i]}</span>
              </div>
...
const ExplorerView = ({ onBack }) => {
  ...
  if (sel) {
    ...
    return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title={`Block #${sel.index}`} onBack={() => setSel(null)} />
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        <Card gradient="bg-gradient-to-br from-cyan-900/20 to-[#0f1f18] border-cyan-800/15">
          <p className="text-[10px] text-[#B8E6FF]/35 font-mono break-all">{sel.hash||'—'}</p>
          <div className="grid grid-cols-2 gap-2 mt-2">
            <div><p className="text-[9px] text-[#B8E6FF]/35 uppercase">Time</p><p className="text-white text-[11px]">{new Date(sel.timestamp*1000).toLocaleString()}</p></div>
            <div><p className="text-[9px] text-[#B8E6FF]/35 uppercase">Miner</p><p className="text-purple-400 text-[11px]">🔒 PoET Anonymous</p></div>
          </div>
        </Card>
        {reward > 0 && <Card><div className="flex justify-between items-center"><span className="text-[#B8E6FF]/50 text-xs">⛏️ {miners} miner{miners>1?'s':''}</span><span className="text-cyan-400 font-bold font-mono">{reward.toFixed(2)} LAC</span></div></Card>}
        <p className="text-[#B8E6FF]/40 text-[11px] font-medium">📋 Transactions ({txs.length})</p>
          ...
          txs.map((tx,i) => {
            ...
            return (<Card key={i}>
              ...
              <div className="grid grid-cols-2 gap-1 text-[10px]">
                <div><span className="text-[#B8E6FF]/35">From: </span><span className={anon?'text-purple-400 italic':'text-[#B8E6FF]/50 font-mono'}>{anon?'🔒 Anonymous':sAddr(tx.from||'')}</span></div>
                <div><span className="text-[#B8E6FF]/35">To: </span><span className={anon?'text-purple-400 italic':'text-[#B8E6FF]/50 font-mono'}>{anon?'🔒 Anonymous':sAddr(tx.to||'')}</span></div>
                {tx.amount>0 && <div><span className="text-[#B8E6FF]/35">Amount: </span><span className={anon?'text-purple-400 italic':'text-cyan-400 font-mono font-bold'}>{anon?'🔒 Hidden':fmt(tx.amount)+' LAC'}</span></div>}
                {tx.unlock_block && <div><span className="text-[#B8E6FF]/35">Unlock: </span><span className="text-blue-400 font-mono">Block #{tx.unlock_block}</span></div>}
              </div>
            </Card>);
          })}
        {msgs > 0 && <>
          <p className="text-[#B8E6FF]/40 text-[11px] font-medium">💬 L2 Ephemeral ({msgs})</p>
          <Card><p className="text-purple-400/70 text-[11px] text-center">🔒 {msgs} encrypted messages — auto-delete 5min</p></Card>
        </>}
      </div></div>);
  }

  return (<div className="h-full bg-[#0B1426] flex flex-col"><Header title="⛓ Explorer" onBack={onBack} right={<Badge>#{h} · last 200</Badge>} />
    <div className="flex-1 overflow-y-auto p-4">
      {loading?<p className="text-[#B8E6FF]/35 text-center py-8">Loading blocks…</p>:
        blocks.length===0?<p className="text-[#B8E6FF]/35 text-center py-8">No blocks</p>:
        blocks.map(b => {
          ...
          return (<Card key={b.index} className="mb-2" onClick={() => setSel(b)}>
            <div className="flex justify-between items-center mb-1">
              ...
              <span className="text-[#B8E6FF]/35 text-[11px]">{ago(b.timestamp)}</span>
            </div>
            <div className="flex gap-3 text-[11px] mb-1">
              <span className="text-[#B8E6FF]/40">{txs.length} tx</span>
              {msgs > 0 && <span className="text-purple-400/60">{msgs} msg</span>}
              <span className="text-[#B8E6FF]/35">🔒 PoET</span>
            </div>
    ...
    <div className="px-4 space-y-2">
      {[
        ...
        <button key={item.type} onClick={() => onNav({type:item.type})}
          className="w-full bg-[#0d1b35] border border-white/[0.05] rounded-2xl p-4 flex items-center gap-4 active:bg-white/[0.04]">
          <span className="text-2xl">{item.icon}</span>
          <div className="text-left flex-1"><p className="text-white font-medium text-[14px]">{item.title}</p><p className="text-[#B8E6FF]/35 text-xs">{item.sub}</p></div>
